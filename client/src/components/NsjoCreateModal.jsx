...
export default function NsjoCreateModal({ nssoId, lineId, onClose, onSaved }) {
  ...
  useEffect(() => {
    Promise.all([
      api.get(`/non-standard-sales-orders/${nssoId}/lines/${lineId}/jo-draft`),
      api.get('/non-standard-sales-orders/meta'),
    ]).then(([d, m]) => {
      setDraft(d.data.line);
      setMeta(m.data);
  ...
  async function save() {
    ...
    try {
      ...
      const { data } = await api.post(`/non-standard-sales-orders/${nssoId}/lines/${lineId}/create-jo`, {
        reason_code_id: reasonCode?.id || null, reason: reason || null, action_to_be_taken: action || null, processes,
      });
      onSaved(data);
  ...
  return (
    <Modal title="Job Order" onClose={onClose} xl>
      {error && <div className="error-banner">{error}</div>}
      {!draft ? <LoadingSpinner /> : (
        <div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, lineHeight: 1.8, marginBottom: 16 }}>
            <div>
              <div>Customer : <span className="hi">{draft.customer_name || '—'}</span></div>
              <div>Contact Person : <span className="hi">{draft.contact_person_name || ''}</span></div>
              <div>Contact Phone : <span className="hi">{draft.contact_phone || ''}</span></div>
            </div>
            <div>
              <div>Office Location : <span className="hi">{draft.office_location_name || ''}</span></div>
              <div>Sales Division : <span className="hi">{draft.sales_division_name || ''}</span></div>
              <div>Sales Rep : <span className="hi">{draft.sales_rep_name || ''}</span></div>
              <div>JO Ref. # : <span className="hi">{draft.source_job_order_no || ''}</span></div>
            </div>
            <div>
              <div>Job Type : <span className="hi">{draft.job_type_name || ''}</span></div>
              <div>Job Desc. : <span className="hi">{draft.description || ''}</span></div>
              <div>Job Location : <span className="hi">{draft.job_location_name || ''}</span></div>
              <div>Quantity : <span className="hi">{Number(draft.quantity)} {draft.units || ''}</span></div>
              <div>Length : <span className="hi">{draft.length ?? 0}</span> Width : <span className="hi">{draft.width ?? 0}</span> Height : <span className="hi">{draft.height ?? 0}</span></div>
            </div>
          </div>
