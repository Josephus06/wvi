...
// All RWIP (rework-in-progress) job orders across the system -- read-only list under Production.
// RWIPs are created/approved/completed from the Production job-order view; this just browses them.
export default function RwipJobOrders() {
      ...
      <div className="card" style={{ marginBottom: 16 }}>
        <div className="filter-grid">
          <div className="field">
            ...
            <input value={search} onChange={(e) => setSearch(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && runSearch()} placeholder="RWIP #, Mother JO #, SO # or Customer..." />
          </div>
          <div className="field">
            <label>Status</label>
        ...
        {loading ? <LoadingSpinner /> : (
          <div className="table-wrap">
            <table className="responsive-cards">
              <thead>
                <tr>
                  <th>RWIP #</th><th>Date</th><th>Mother JO</th><th>Sales Order</th><th>Customer</th>
                  <th>Job Type</th><th>Description</th><th style={{ textAlign: 'right' }}>Qty</th><th>Approved By</th><th>Status</th><th></th>
                </tr>
              </thead>
              <tbody>
                {rows.length === 0 && <tr><td colSpan={11} className="muted" style={{ textAlign: 'center', padding: 20 }}>No RWIP job orders found.</td></tr>}
                {pageRows.map((row) => (
                  <tr key={row.id}>
                    <td data-label="RWIP #">{row.job_order_no}</td>
                    ...
                    {/* Pending approval -> Sales JO view (Approve RWIP lives there); once approved it's
                        in production, so link into the Production view to Build / Quality Inspect it. */}
