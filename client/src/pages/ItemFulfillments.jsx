...
// Global "Saved Item Fulfillments" list -- every Item Fulfillment across every Transfer
// Order, filterable the same way the real system's own screen is. Previously these were
// only reachable one Transfer Order at a time (its Related Records tab); this gives the
// same standalone browse the real system has. Creating one still only happens from a
// specific Transfer Order's "Fulfill" button -- that flow already exists and doesn't
    ...
    <div>
      <div className="page-header">
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 16 }}>
          <h1 style={{ fontSize: 16, textTransform: 'uppercase', margin: 0 }}>Saved Item Fulfillments</h1>
        </div>
      </div>
        ...
        {loading ? <LoadingSpinner /> : (
          <div className="table-wrap">
            <table className="responsive-cards">
              ...
              <tbody>
                {rows.length === 0 && (
                  <tr><td colSpan={9} className="muted" style={{ textAlign: 'center', padding: 20 }}>No item fulfillments found.</td></tr>
                )}
                {rows.map((row) => (
                  <tr key={row.id}>
                    <td data-label="Item Fulfillment #">{row.fulfillment_no}</td>
                    ...
                    <td><Link className="btn btn-sm btn-primary" to={`/transfer-orders/item-fulfillments/${row.id}`}>View</Link></td>
                  </tr>
                ))}
              </tbody>
            </table>
