// One-off migration: registers the "RWIP" list page under the Production module, so all RWIP
// (rework-in-progress) job orders raised off in-process mother JOs can be browsed in one place.
// RWIPs are job_orders rows with parent_job_order_id set (see alter-job-rwip.js) -- this page just
// lists them; they're created from the Production JO view's RWIP JO tab and approved there.
//
//   node src/db/create-rwip-page.js --dry-run
//   node src/db/create-rwip-page.js
const pool = require('../db');
require('dotenv').config();

const DRY_RUN = process.argv.includes('--dry-run');
const PAGES = [{ route: '/rwip-job-orders', name: 'RWIP', module: 'Production' }];
